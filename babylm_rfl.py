...
#   print('Training...')
	...
	for step, batch in enumerate(train_dataloader, start=1):
		...
		# loss = criterion(pred, label) # Compute loss function
		...
		if step % 300 == 0:
			print(f"{current_time_train} - epoch {epoch+1:1d} - iter {step:1d}/{len(train_dataloader):1d} - train/loss {loss.item():.8f} - lr: {lr_scheduler.get_lr()[0]:.6f}")
...
#           )
		...
		loss = loss / gradient_accumulation_steps # normalizing the loss based on the number of accumulation steps 
...
def main():
	...
	EPOCHS = 5
	LEARNING_RATE = 1e-3 # 2e-5 # 5e-4 # 6e-4
	weight_decay = 0.1
...
#   print(f"{current_time_main} - Parameters: {config}")
	print(f"{current_time_main} - {'-' * 89}") 
	print(f"{current_time_main} - Model training base path:: {output_dir}")
	print(f"{current_time_main} - {'-' * 89}") 
